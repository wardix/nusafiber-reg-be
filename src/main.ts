...
// Validation schemas
const homepassIdSchema = z.string()
  .regex(/^[A-Z0-9]{4}-[A-Z0-9]{5}-H[0-9]{5}$/, 'Format Homepass ID tidak valid');

const phoneSchema = z.string()
  .min(10, 'Nomor handphone minimal 10 digit')
  .max(13, 'Nomor handphone maksimal 13 digit')
  .regex(/^0[0-9]+$/, 'Nomor handphone harus dimulai dengan 0 dan hanya berisi angka');
...
app.post('/api/register', async (c) => {
  try {
    ...
    // Basic validation
    if (!homepassId || !customerName || !phoneNumber || !locationStr || !ktpFile) {
      return c.json({ 
        success: false,
        error: 'Missing required fields',
        details: 'All fields (homepassId, customerName, phoneNumber, location, ktpFile) are required'
      }, 400);
    }
